...
TEST_F(TupleTest, SerializeInto_Integer_WritesCorrectBytes) {
  ...
  char buf[16] = {};
  uint32_t out_size = 0;
  ASSERT_TRUE(t.serialize_into(schema, buf, sizeof(buf), &out_size));
...
TEST_F(TupleTest, SerializeInto_Varchar_Writes2ByteLengthPrefixThenData) {
  ...
  uint32_t out_size = 0;
  ASSERT_TRUE(t.serialize_into(schema, buf, sizeof(buf), &out_size));
  EXPECT_EQ(out_size, 2u + name.size());

  uint16_t len;
  std::memcpy(&len, buf, sizeof(uint16_t));
  EXPECT_EQ(len, static_cast<uint16_t>(name.size()));
  EXPECT_EQ(std::memcmp(buf + 2, name.data(), name.size()), 0);
}
...
TEST_F(TupleTest, Serialize_ReturnsHeapBuffer_WithCorrectContent) {
  Schema schema = make_numeric_schema();
  Tuple t(std::vector<Value>{int32_t{7}, double{1.5}});

  uint32_t out_size = 0;
  char* buf = t.serialize(schema, &out_size);
  ASSERT_NE(buf, nullptr);
  EXPECT_EQ(out_size, 12u);  // 4 + 8

  int32_t id;
  double score;
  std::memcpy(&id, buf, sizeof(int32_t));
  std::memcpy(&score, buf + 4, sizeof(double));
  EXPECT_EQ(id, 7);
...
TEST_F(TupleTest, SerializeInto_BufferTooSmall_ReturnsFalse_SetsRequiredSize) {
  ...
  char buf[4] = {};
  uint32_t out_size = 0;
  bool ok = t.serialize_into(schema, buf, sizeof(buf), &out_size);
  EXPECT_FALSE(ok);
  EXPECT_EQ(out_size, 12u);
}
...
TEST_F(TupleTest, SerializeInto_ValueCountMismatch_Throws) {
  ...
  char buf[64] = {};
  uint32_t out_size = 0;
  EXPECT_THROW(t.serialize_into(schema, buf, sizeof(buf), &out_size), std::runtime_error);
}

TEST_F(TupleTest, Deserialize_Integer_ReturnsCorrectValue) {
  Schema schema({Column("n", DataType::INTEGER, 4, 0)});
  int32_t raw = 42;
  Tuple t = Tuple::deserialize(schema, reinterpret_cast<const char*>(&raw), sizeof(raw));
  ASSERT_EQ(t.size(), 1u);
...
TEST_F(TupleTest, Deserialize_BooleanTrue_ReturnsTrueValue) {
  ...
  char raw = 1;
  Tuple t = Tuple::deserialize(schema, &raw, 1);
  ASSERT_EQ(t.size(), 1u);
...
TEST_F(TupleTest, Deserialize_BooleanNonZero_ReturnsTrueValue) {
  ...
  Schema schema({Column("b", DataType::BOOLEAN, 1, 0)});
  char raw = static_cast<char>(0xFF);
  Tuple t = Tuple::deserialize(schema, &raw, 1);
  EXPECT_TRUE(std::get<bool>(t.get_value(0)));
}
...
TEST_F(TupleTest, Deserialize_Varchar_ReturnsCorrectString) {
  ...
  uint16_t len = 5;
  char buf[16] = {};
  std::memcpy(buf, &len, sizeof(uint16_t));
  std::memcpy(buf + 2, word, 5);
...
TEST_F(TupleTest, RoundTrip_NumericSchema_PreservesAllValues) {
  ...
  uint32_t out_size = 0;
  char* buf = original.serialize(schema, &out_size);
